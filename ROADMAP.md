...
| v0.11.0 | **The spec** — `docs/spec/`: manifest, contract, evaluation, gates, tickets, drive scripts, and the role hook contract written as format specifications independent of the Python implementation; warts surfaced by spec review fixed before the 1.0 freeze. The standard play: neutral formats other harnesses can emit and consume. Alongside it: `darkroom dossier` — deterministic assembly of the cross-run record (iterations, per-criterion score trajectories, checkpoint subjects, escalations, gate movements) into one operator-facing bundle, the substrate for a narration skill that surfaces sticking points and novelties (shipped as `skills/darkroom-chronicle/`, alongside usage metering — the cost ledger) | Complete |
| v0.12.0 | **Browser steps** — the drive vocabulary grows `goto`/`click`/`fill`/`screenshot` step kinds (Playwright behind the existing extra), extending the exam engine to visual tenants; screenshots flow through the existing producer, so no new evidence kind — the drive-script spec's first minor bump (1.1) exercised the versioning policy as written. Shipped with the example tenant's UI slice and the CI-tested quickstart | Complete |
| v0.15.0 | **Exam authoring safety** — the chronicle's systemic finding (≈27% of the first tenant's spend traced to operator authoring defects, zero to builder failures) answered as tooling: `darkroom audit`, the witnessability lint (rubric evidence kinds statically checked against what each drive can produce); the blocker protocol (a builder disputing the exam pauses model escalation — a signal that may only *reduce* resources spent on the builder, never halt, score, or gate, per the codified invariant in role-hooks 1.1); and the interview skill's hard rules (name the witness; exams assert machine surfaces, judges assess presentation) | Complete |
| v0.16.0 | **Expect witnesses** — browser and http step `expect` outcomes recorded in the manifest as evidence, not just enforced: a `click` whose expect requires a selector or a `goto` whose expect requires a body value writes what it checked and what it found into the log the judge reads. Field-driven: three builder blockers in one tenant (flagged_not_me, the landing exit, passkey_signin) were all this class — the harness enforcing a value the evidence never showed — each costing an operator round-trip that this retires wholesale | Proposed |
...
- **Calibration tooling** — reliability trials and gaming audits as
  commands; gate credibility as a measured property, not a promise.
- **The operator console** (`darkroom ui`) — gate trends, run browser,
  the chronicle as its narrative layer.
- **A second implementation** — the evidence-side subset (drive,
  verify, gates) built against `docs/spec/` rather than this codebase,
  plausibly as a native binary for polyglot tenants; a standard is
  proven by its second implementation, and the spec exists precisely
